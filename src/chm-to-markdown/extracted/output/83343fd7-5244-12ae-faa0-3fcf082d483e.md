# CanLeaderEndConditionBeAssigned Method

﻿
 Code: All Code: Multiple Code: C# Code: Visual Basic Code: Visual C++   
---  
C#Visual BasicVisual C++
Revit 2024 API  
---  
IndependentTag..::..CanLeaderEndConditionBeAssigned Method   
[IndependentTag Class](e52073e2-9d98-6fb5-eb43-288cf9ed2e28.md "IndependentTag Class") See Also  
---  
Checks whether the LeaderEndCondition can be changed. 
**Namespace:** [Autodesk.Revit.DB](87546ba7-461b-c646-cbb1-2cb8f5bff8b2.md "Autodesk.Revit.DB Namespace")**Assembly:** RevitAPI (in RevitAPI.dll) Version: 24.0.0.0 (24.0.0.0)
# Syntax
C#  
---  
```text
public bool CanLeaderEndConditionBeAssigned(
	LeaderEndCondition leaderEndCondition
)
```
  
Visual Basic  
---  
```text
Public Function CanLeaderEndConditionBeAssigned ( _
	leaderEndCondition As LeaderEndCondition _
) As Boolean
```
  
Visual C++  
---  
```text
public:
bool CanLeaderEndConditionBeAssigned(
	LeaderEndCondition leaderEndCondition
)
```
  
# ### Parameters
leaderEndCondition
    Type: [Autodesk.Revit.DB..::..LeaderEndCondition](c86e3feb-80c8-ab4c-e9cf-18875131b96a.md "LeaderEndCondition Enumeration") The leader end condition to check. 
# ### Return Value
True if the leader end condition of the tag can be assigned, or false otherwise. 
# Remarks
Material tags and material keynotes can only use the free end condition. 
# Exceptions
| Exception | Condition |
| --- | --- |
| --- | --- |
| [Autodesk.Revit.Exceptions..::..ArgumentOutOfRangeException](60f148c9-ece0-a6bb-4e12-bb4a9c8c8a24.md "ArgumentOutOfRangeException Class") | A value passed for an enumeration argument is not a member of that enumeration |

# See Also
[IndependentTag Class](e52073e2-9d98-6fb5-eb43-288cf9ed2e28.md "IndependentTag Class")
[Autodesk.Revit.DB Namespace](87546ba7-461b-c646-cbb1-2cb8f5bff8b2.md "Autodesk.Revit.DB Namespace")
Send comments on this topic to [Autodesk](mailto:revitapifeedback%40autodesk.com?Subject=Revit 2024 API)