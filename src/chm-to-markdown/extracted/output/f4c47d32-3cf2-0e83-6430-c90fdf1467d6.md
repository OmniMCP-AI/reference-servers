# SphericalLightDistribution Constructor (SphericalLightDistribution)

﻿
 Code: All Code: Multiple Code: C# Code: Visual Basic Code: Visual C++   
---  
C#Visual BasicVisual C++
Revit 2024 API  
---  
SphericalLightDistribution Constructor (SphericalLightDistribution)  
[SphericalLightDistribution Class](978d468b-4c4a-d439-3d87-bd79e211887d.md "SphericalLightDistribution Class") See Also  
---  
Creates a copy of the given spherical light distribution 
**Namespace:** [Autodesk.Revit.DB.Lighting](a6a04f07-7fd2-0a4e-12e7-01842ee6daaf.md "Autodesk.Revit.DB.Lighting Namespace")**Assembly:** RevitAPI (in RevitAPI.dll) Version: 24.0.0.0 (24.0.0.0)**Since:** 2013 
# Syntax
C#  
---  
```text
public SphericalLightDistribution(
	SphericalLightDistribution other
)
```
  
Visual Basic  
---  
```text
Public Sub New ( _
	other As SphericalLightDistribution _
)
```
  
Visual C++  
---  
```text
public:
SphericalLightDistribution(
	SphericalLightDistribution^ other
)
```
  
# ### Parameters
other
    Type: [Autodesk.Revit.DB.Lighting..::..SphericalLightDistribution](978d468b-4c4a-d439-3d87-bd79e211887d.md "SphericalLightDistribution Class")
# Exceptions
| Exception | Condition |
| --- | --- |
| --- | --- |
| [Autodesk.Revit.Exceptions..::..ArgumentNullException](631e1424-60f4-929b-4e52-dda9dcd26316.md "ArgumentNullException Class") | A non-optional argument was null |

# See Also
[SphericalLightDistribution Class](978d468b-4c4a-d439-3d87-bd79e211887d.md "SphericalLightDistribution Class")
[SphericalLightDistribution Overload](c8282dc5-ef4d-27fd-e885-af0d2d6e193d.md "SphericalLightDistribution Constructor")
[Autodesk.Revit.DB.Lighting Namespace](a6a04f07-7fd2-0a4e-12e7-01842ee6daaf.md "Autodesk.Revit.DB.Lighting Namespace")
Send comments on this topic to [Autodesk](mailto:revitapifeedback%40autodesk.com?Subject=Revit 2024 API)