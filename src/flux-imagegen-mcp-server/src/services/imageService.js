...
export async function downloadImage(imageUrl, fileName = '', format = 'png') {
  try {
    ...
    if (!fileName) {
      ...
      fileName = `downloaded-image-${timestamp}`;
    }

    // Clean filename and ensure it's unique
    fileName = fileName.replace(/[^a-zA-Z0-9-_]/g, '-');
    let finalPath = path.join(imagesDir, `${fileName}.${format}`);
    let counter = 1;
    
    while (fs.existsSync(finalPath)) {
      finalPath = path.join(imagesDir, `${fileName}_${counter}.${format}`);
      counter++;
    }
